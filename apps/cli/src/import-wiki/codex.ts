...
export interface GenerateImageOptions {
  ...
  dryRun?: boolean;
  outputPath: string;
  /** Custom shell runner — injected by tests. Defaults to `codex exec` via Bun.spawn. */
  runner?: (prompt: string) => Promise<{ stdout: string; stderr: string }>;
  /**
   * Directory inside codex's sandbox where the PNG is written before being
   * moved to `outputPath`. Required because codex's `workspace-write` sandbox
   * blocks writes outside its workdir (the CLI app), so we cannot point it
   * directly at the blog's assets dir.
   */
  stagingDir: string;
  title: string;
}
...
/**
 * Generate a hero image via the Codex CLI.
 *
 * Single codex session: read article body inline → compose visual prompt →
...
export async function generateHeroImage(
  ...
  if (dryRun) {
    console.log(
      `[codex] DRY_RUN — would generate image for "${title}" → ${stagingPath} → ${outputPath}`
    );
    return;
  }
  ...
  if (stderr.trim()) {
    console.warn(`[codex] stderr for "${title}":\n${stderr}`);
  }
  if (stdout.trim()) {
    console.log(`[codex] stdout for "${title}":\n${stdout}`);
  }
