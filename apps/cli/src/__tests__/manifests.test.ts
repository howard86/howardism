...
describe("buildManifests", () => {
  ...
  it("harvests each concept page's own Open Questions section", async () => {
    const dir = await mkdtemp(join(tmpdir(), "manifests-oq-"));
    const concept = makeParsed(
      "agent-loop-pattern",
      "## Open Questions\n\n- Who owns the budget?\n- Still need a backlog?\n\n## Sources\n\n- not a question\n"
    );
    const set = await buildManifests(buildArgs([concept], dir));
  ...
  it("ignores the vault's generated backlog digest", async () => {
    const dir = await mkdtemp(join(tmpdir(), "manifests-oq-digest-"));
    const backlog = makeParsed(
      ...
      [
        ...
        "",
        "- [[agent-loop-pattern]] ×2 (oldest 1d) — Who owns the budg…",
        "",
    ...
    const set = await buildManifests(buildArgs([backlog, concept], dir));

    // The digest truncates every question and is regenerated by the vault's own
    // tooling, so the concept page is the only source the manifest reads.
    expect(set.openQuestions.byConcept).toHaveLength(1);
    expect(set.openQuestions.byConcept[0]).toMatchObject({
      slug: "agent-loop-pattern",
