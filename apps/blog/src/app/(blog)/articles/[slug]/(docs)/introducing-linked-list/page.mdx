...
description: Linked lists are a fundamental data structure in computer science.
  They are often used to represent a sequence of elements, where each element is
  linked to the next one. In this post, we'll introduce linked lists and explore
  some algorithm questions related to them.
tag: Essay
readingTime: 3
imageAlt: cat that is stepping on a chain
archived: true
---
export { default as heroImage } from "../(assets)/cat-stepping-on-chain.png";
