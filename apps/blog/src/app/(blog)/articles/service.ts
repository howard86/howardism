...
/* ── quiz bank (generated by `bun run generate:quiz`) ── */

export type { QuizConcept } from "@howardism/article-contract/manifests/quiz";

const quiz = parseQuiz(quizData);
const quizBySlug = new Map(
  quiz.concepts.map((concept) => [concept.slug, concept])
);

/** The quiz bank for one article, or undefined when none was generated. */
export const getQuizConcept = (slug: string): QuizConcept | undefined =>
  quizBySlug.get(slug);

export const hasQuiz = (slug: string): boolean => quizBySlug.has(slug);

/** Every quizzed concept in a domain — the nodes of its skill tree. */
export const getQuizByDomain = (domain: ArticleDomain): QuizConcept[] =>
  quiz.concepts.filter((concept) => concept.domain === domain);

/** Domains that have at least one quizzed concept (drives the learn index). */
export const getQuizDomains = (): ArticleDomain[] => [
  ...new Set(quiz.concepts.map((concept) => concept.domain)),
];

export const getAllQuizConcepts = (): QuizConcept[] => quiz.concepts;
