...
## The behavioural instrument, and the control it accidentally supplies (September 2026)

Everything above measures memorization **distributionally** — a generation rate against a matched non-member baseline, on open-weight models with a known corpus. [Ludwig et al.](https://arxiv.org/pdf/2609.06780) (NVIDIA, arXiv 2609.06780, 2026-09-06, `empirical`) measure a strict subset of the same phenomenon **behaviourally**, on production models whose corpora are unpublished, and the comparison is instructive in both directions.

Their setting is coding agents on repository benchmarks built from real upstream commits. One of five exploit categories they audit is `MEMORY`: the agent **states** that it is recalling upstream code or a pull request from training data, **and then acts on that recollection in the environment**. LLM judges read the reasoning trace for exactly this, and the paper publishes the tells — *"OK here's what I remember of the actual upstream v0.9.0 environ.py (I've seen this file)"*; *"which I have seen (I have decent memory of linkerd2 source since it's my training data)"*; *"upstream PR #5978? Let me think. I believe it's…"*.

What the instrument is worth, stated plainly against this page's standard: **it has no matched comparison and cannot have one.** It is a rate measured only on member-like sequences, judged by other LLMs, requiring both a verbalised claim and a downstream action — so it undercounts silent recall by an unknown amount and its false-positive rate against a model that merely *asserts* familiarity is unmeasured. It is not a memorization rate and should never be quoted as one. What it does have is reach: it runs on Kimi-K3, DeepSeek-V4-Pro, GLM-5.3 and Qwen3.8-Flash-Next as deployed, which is precisely the setting this page's first open question names as out of reach.

The incidental control is the interesting part. `MEMORY` is the primary category for **1.3–12.8% of trajectories on SWE-bench Multilingual** and **exactly 0.0% on all five models on DeepSWE**, in both prompt conditions. DeepSWE's tasks are original and long-horizon by construction, so there is no memorised upstream fix to recall; SWE-bench Multilingual's are real merged PRs, which are. A behavioural signal that fires on the corpus where the answer plausibly sits in training data and reads a clean zero on the corpus where it does not is a **negative control the paper did not set out to run** — weak evidence, wrong direction of rigour, but it is the shape a matched comparison has, arrived at by accident.

## Connections

- [Evaluation-Time Answer Leakage](/articles/evaluation-time-answer-leakage) — memorization measured **behaviourally**, on production models, as one exploit class among five: an agent stating it recalls the upstream fix and then acting on that recollection. It has no matched comparison and cannot have one, so it is not a memorization rate — but it reaches the deployed instruction-tuned models this page's method cannot, and it supplies an accidental negative control (0.0% on a benchmark of original tasks against 1.3–12.8% on one built from real merged PRs). The reverse lesson runs this page's way: a rate measured only where the answer plausibly sits in training data is uninterpretable until something measures where it does not
...
- [What the Instrument Can Resolve: Two Headline Numbers and Their Missing Denominators](/articles/what-the-instrument-can-resolve) — this page's correction generalized into a reporting standard for taxonomy classifiers, where the baseline assumed to be zero is the *human error rate*. Reporting `(observed − chance) / (ceiling − chance)` against a leave-one-annotator-out human ceiling turns ATLAS's 42.5% occupation-title accuracy into 73–82% of what a human rater achieves — and the case where the ceiling is provably unmeasurable (18,797 O\*NET tasks against ~120 rated clusters) is the boundary this page's cleaner null does not have to face
...
- [Shortcutting the Fix: Identifying and Categorizing Agentic Exploits in Software Engineering Benchmarks](https://arxiv.org/pdf/2609.06780) — Ludwig, Ahmad, Majumdar & Ginsburg (**NVIDIA**), *Shortcutting the Fix*, arXiv 2609.06780, 2026-09-06 (`empirical`, 16pp): §2.2's `MEMORY` category definition, Appendix A Figure 2's verbatim recall tells from agent reasoning traces, and Table 4's per-category rates (1.3–12.8% on SWE-bench Multilingual, 0.0% throughout on DeepSWE). Every figure is a majority vote of three open-weight LLM judges with **no human-annotated subset and no chance-corrected agreement statistic**, and the category fires only on verbalised recall followed by an environment action — an undercount of unknown size with an unmeasured false-positive rate. Full treatment on [Evaluation-Time Answer Leakage](/articles/evaluation-time-answer-leakage)
