...
---
export { default as heroImage } from "../assets/ticket-driven-agent-orchestration.webp";
...
- [Agent Context Files](/articles/agent-context-files) — `WORKFLOW.md` is the orchestration-layer instance of the markdown-as-control-plane pattern; the ticket layer invokes the policy plane that context files define
- [Loop Engineering](/articles/loop-engineering) — a Linear board (or markdown file) is the loop's sixth primitive, *memory* — the durable work graph that lets a morning automation resume where yesterday's run stopped; ticket-driven orchestration is this state primitive made first-class
- [Agentic Work Systematization](/articles/agentic-work-systematization) — tickets are the work-graph side of externalizing agent context; skills/plugins are the procedure side — both turn ad-hoc delegation into repeatable, handed-off workflows
- [Orchestration-Plan Simulation](/articles/orchestration-plan-simulation) — the benchmarked form of this page's dependency graph, and the scope line between them. OrchBench takes a task DAG as a *fixed input* and evaluates only what a planner does with it (which agent runs which subtask, and what information crosses between agents), which is exactly the half Symphony leaves to the agents: Symphony's `blocked_by` graph is agent-extensible and grows during execution, while OrchBench freezes it so plan quality can be isolated. Its finding is the one this page's model makes easy to overlook — **the dependency edge is not free**. When one ticket's output is a prerequisite for another assigned elsewhere, something has to carry the result across, and in OrchBench's measurements the omitted or over-compressed handoff is where essentially all the quality goes, far more than agent count. A tracker that records `blocked_by` but not *what gets passed* is recording half the plan

## Derived

- [Agent Control Plane Patterns: Tickets, Loops, Specs, and Memory Files](/articles/agent-control-plane-patterns) — positions tickets as the primary durable work graph, loops as execution, specs/context files as policy, and memory as bounded recall
