...
- [Skill Lift](/articles/skill-lift) — an instrument for rung 3 of the ladder. The ladder says customize at the highest layer that works; NVIDIA's with/without skill ablation is how you find out whether the skills-and-plugins rung actually worked before descending to the SDK. Its cross-harness result also prices the rung-4 decision from the other side: per-*product* Skill Lift ranges +2 to +46 while the two harnesses differ by ~5 points, i.e. the artifact you author matters far more than which runtime executes it
