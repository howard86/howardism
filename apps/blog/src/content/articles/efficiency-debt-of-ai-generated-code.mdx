...
- [The Committed-Artifact Chain](/articles/committed-artifact-chain) — the prescription this page's numbers bear on. Anthropic's Applied AI SDLC playbook (`vendor-claim`, 2026-08-21) predicts review time per PR falls *"once the tests catch what reviewers used to catch"*; the 1.92× blocking-thread ratio measured here is the current state it is predicting away from, and this page's own null — review depth does not predict which inefficiencies survive — is the harder problem for it, since the playbook's answer to review load is more and better review passes at the same join. The two agree on where the fix belongs (upstream of review, in what gets authored) and disagree on whether the mechanisms proposed reach it.
- [The Code-Quality Payoff Is Token-Indexed](/articles/code-quality-payoff-is-token-indexed) — the counter-case to a purely token-budget account of code quality: this page's debt never matures into a rewrite, it bills monthly in compute, so a larger context budget does not dissolve it
