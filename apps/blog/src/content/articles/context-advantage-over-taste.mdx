...
- [GDPval Benchmark](/articles/gdpval-benchmark) — the asymmetry run as an experiment on paid work. GDPval writes all the context a 10-year professional carries in their head into the prompt, and the model's win rate against that professional reaches ~48%; **remove** the context again and the win rate slips a few points while the qualitative failure is larger — *"the models actually struggle to figure out what to work on."* The lecturer's own gloss is this page's thesis in occupational form: *"humans are basically architecting what is the set of problems and then the model can go solve it"*
