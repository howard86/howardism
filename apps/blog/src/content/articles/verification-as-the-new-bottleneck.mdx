...
This page's bottleneck has a proposed dissolution, and it is the obvious one: if review is what saturates, hand review to agents. [DHH](/articles/dhh) reports doing it wholesale ([Lex Fridman #501](https://www.youtube.com/watch?v=NYFGCESmikA), 2026-08-26, `practitioner-opinion`) — agents triage every incoming PR to his Linux distro and return only a merge-or-not summary ([Open Source Under Agent Contributions](/articles/open-source-under-agent-contributions)), and cross-model review is standing procedure on his own work ([Same-Model Review Blindness](/articles/same-model-review-blindness)).
...
[Robert C. Martin](/articles/robert-c-martin) reaches the same destination as DHH above — the human stops reading the diff — by a route that does not require trusting an agent reviewer at all ([Uncle Bob on Software Fundamentals in the Age of AI](https://www.youtube.com/watch?v=zcLPGC-tvgk), 2026-08-19, `practitioner-opinion`). His stated goal is unusually blunt:
...
What replaces reading is a stack of must-pass deterministic gates — coverage-plus-complexity scoring, mutation testing to kill surviving mutants, a dependency-rule checker, an executable QA script driving the UI ([Reviving Impractical Quality Tools](/articles/reviving-impractical-quality-tools)). The residual human activity is deliberately not review: "I will look at the CRAP scores and make sure that they're low. And I will do spot checks on the code from time to time."

Three reasons this is a distinct answer rather than a variant of DHH's:

- **It does not inherit the reviewer-blindness problem.** Agent review of agent code runs into shared blind spots ([Same-Model Review Blindness](/articles/same-model-review-blindness)) and missing project context ([Agent Review Comment Resolution](/articles/agent-review-comment-resolution)). A mutation tester has no blind spots to share — it is the same class of thing as a compiler, which is why [Verifying Without a Compiler: Cowork's Harness vs Claude Code's, and Why the Slice Verifier Stays](/articles/verifying-without-a-compiler) frames the whole difficulty as the absence of one.
...
The unaddressed gap is the one this page keeps returning to: gates verify what someone thought to check. Martin's own account concedes the residual — spot checks, and an architecture step he still does by hand ([Deep Modules for Agents](/articles/deep-modules-for-agents)) — so what he has actually removed is *line-level* review, not verification. And it is one practitioner's unmeasured report, grading tools he wrote himself.

## Connections

- [Robert C. Martin (Uncle Bob)](/articles/robert-c-martin) — the mechanical answer to the bottleneck: gates instead of a delegated reviewer, with a stated price and a stated stopping rule
- [Reviving Impractical Quality Tools](/articles/reviving-impractical-quality-tools) — what goes in the gate stack, and why those tools are available now
- [Impose Values, Not Disciplines](/articles/impose-values-not-disciplines) — the corollary for TDD, which this page already records as losing its tax
...
- [DHH (David Heinemeier Hansson)](/articles/dhh) — the practitioner arguing the bottleneck is delegable, with a second-hand Shopify incident-tracing study as the only evidence
...
- [DHH: Future of Programming, AI, Agentic Engineering, Vibe Coding & Linux | Lex Fridman Podcast #501](https://www.youtube.com/watch?v=NYFGCESmikA) — DHH, Lex Fridman #501 (2026-08-26, `practitioner-opinion`, second-hand): the Shopify incident-to-PR study claiming agent-reviewed PRs caused fewer production issues; "agents are better at finding bugs"
...
- [Uncle Bob on Software Fundamentals in the Age of AI](https://www.youtube.com/watch?v=zcLPGC-tvgk) — Robert C. Martin with [Matt Pocock](/articles/matt-pocock), 2026-08-19 (`practitioner-opinion`; auto-caption transcript): "I don't have to look at the code at all" via a must-pass gate stack rather than a delegated agent reviewer, priced at ~12x a bare agent run and with a named stopping rule
