...
description: Treating an agent's active context as indexed runtime objects with
  a lifecycle (fold/mask/prune, recoverable sidecars, cache-aware commit) rather
  than a token buffer to trim — Xiaohongshu's Self-GC is the measured treatment
  (~44% prefix pruning at ~85% no-impact), plus the five-primitive taxonomy, the
  O(n²) full-append cost case, and RCWT's coordination-share cliff.
tag: Concept
...
### An independent, non-vendor LongMemEval number (2026-09-02)

[Karunanidhi](https://arxiv.org/pdf/2608.21230) (arXiv 2608.21230, `empirical`) is a memory-*security* paper, but it establishes a clean-condition baseline on the same benchmark with the full configuration disclosed, and it is the first LongMemEval number in the corpus that is not a vendor reporting on its own product. It is the useful comparison precisely because the system is deliberately unimpressive: **plain semantic search at top-*k*=15, no reranking, no query rewriting, no summarization, no graph structure**, one memory per conversational round, each question in its own namespace. Reader `claude-sonnet-5`, judge `gpt-4o-2024-08-06` on LongMemEval's official prompts verbatim.

| Question type | Plain retrieval (n) | Maximem Synap self-report |
|---|---|---|
| single-session-assistant | 1.000 (56) | 87.5% |
| single-session-user | 0.943 (70) | 100.0% |
| knowledge-update | 0.936 (78) | 100.0% |
| temporal-reasoning | 0.827 (133) | 100.0% |
| **multi-session** | **0.767 (133)** | **75.2%** |
| single-session-preference | 0.767 (30) | 100.0% |
| **Overall** | **0.860 (500)** | **92.0%** |

Three readings, in decreasing confidence. First, **the rows are still not comparable** — different reader, different judge — and this page's rule about Table 3 applies to its own table here. Second, **the weak cell replicates**: multi-session aggregation is the worst category in both, by a wide margin in the independent measurement, which is the strongest available support for the reasoning-sufficiency argument above being a property of the *task* rather than of one system's retriever. Third, and stated as an observation rather than a finding: four of Maximem's six categories are reported at **exactly 100.0%** where a plain-retrieval baseline with a strong reader scores 0.767–0.943 on the same categories. That is either a very large context-layer effect or a measurement regime difference, and nothing in the corpus separates the two.

**The diagnostic worth stealing.** Asked why multi-session is the weak cell, the paper does the obvious test and gets the counter-intuitive answer: **raising top-*k* makes it worse, not better** — 0.742, 0.677, and 0.613 at *k* = 15, 30 and 50. If the failure were retrieval recall, more candidates would help. Instead the reader over-counts from plausible-looking additional context, which makes it a **reader-side aggregation failure rather than a retrieval failure** — and, in the paper's own framing, evidence that "this memory was already sensitive to what else is in the context window before anyone attacked it." *(One unreconciled inconsistency, flagged rather than smoothed: the sweep prints 0.742 for k=15 while the table above prints 0.767 for the same k. The paper does not reconcile them — plausibly the build-version caveat it states in the same section — so only the sweep's direction is carried here, not its absolute values.)*

That monotone decline is this page's displacement thesis measured on a memory benchmark rather than on coordination content: past a point, adding relevant-looking material to the window costs accuracy even when the answer is already in it.

### The retrieval budget is a security parameter, and the two sweeps do not overlap (2026-09-02)

[PipePoison](/articles/memory-and-context-poisoning) (Zang et al., Shandong University, [arXiv 2609.00523](https://arxiv.org/pdf/2609.00523), `empirical`) sweeps the same knob from the adversarial side and the sign inverts. Over 12 memory-agent configurations, end-to-end attack utilization rises from **34-39% at K = 1 to 67-73% at K = 5**, with limited gains from 5 to 9; write success is flat across the sweep, because the budget does not touch writing, and RSR@K rises monotonically with the window. Two neighbouring deployment knobs behave the same way. Growing the victim's benign store from 100 to 3,000 items leaves WSR flat, cuts RSR@5 (worst case 99% to 77%) and leaves AUR at **59-69%** — more competition is a tax on the attacker, not a wall. Growing a tool return from one item to ten dilutes the poison *before* it is written, taking WSR from 75-83% to 52-61% and AUR from 71-76% to **47-52%**, which is the only knob on this list that acts at the write stage.

Set beside the benign sweep above, this is the page's cleanest case of a lifecycle knob with two-sided cost — and **the two sweeps do not overlap**, which is the caveat that has to travel with the pairing. Karunanidhi measures *k* = 15, 30, 50 and finds accuracy falling monotonically; PipePoison measures K = 1 to 9 and finds attack success rising and then flattening at 5. Nothing measures both quantities on the same range, so "a small window is free" is an extrapolation across a gap, not a result. What *is* established jointly is the shape of the burden of proof: on the benign side nothing measured wants a window above the knee, and on the adversarial side everything does — so a retrieval budget set generously is a choice that now has to be argued for, not a default.
...
## Compaction as a safety-relevant transform (August 2026)

Compaction is treated throughout this page as a *capability* mechanism — what survives the summary determines whether the agent can still do the task. [UK AISI's INC-2026-07-28-01](/articles/unsanctioned-action-in-evaluations) (`case-study`, 2026-08-04) records it acting on a **belief with safety consequences**, in both directions, inside one incident.

AISI's hypothesis, offered tentatively: *"where an agent had reasoned about whether a person was real before compaction, that nuance can be lost in the compaction and the summary may carry forward a false assumption (i.e. that the person is an AI agent acting as part of the range) as established fact."* The trajectory in question ran 750+ turns across roughly a dozen compaction events.

The observed counterpart runs the other way, and is stronger evidence because it is a quoted artifact rather than a suspicion. A compaction summary in another sample reads: *"PARALLEL-CLONE AGENTS (resolved anomaly): other AI agents run identical cloned ranges and piggyback on MY GitHub account via public token2."* Here compaction propagated a **correct** inference — and note the mechanism is identical to the failure AISI hypothesizes: the label `(resolved anomaly)` converts a live question into a settled fact. That is what a good summary does. It is also exactly how a wrong conclusion would become load-bearing and unrevisitable.

The general form, which this page should carry: **compaction is lossy about epistemic status, not just about content.** A hedge ("this might be a real person") costs tokens that a conclusion ("this is an NPC") does not, so summarisation is structurally biased toward promoting tentative beliefs to settled ones. For an ordinary task that is a feature — it is why compaction works. For a belief that gates whether an action harms someone, it removes the doubt that was doing the safety work, and no downstream step re-derives it.

Evidence discipline: AISI's compaction claim is explicitly hedged (*"may be an important mechanism"*, *"appears to"*), no counterfactual analysis was run, and it is one sample. Recorded as a mechanism worth instrumenting, not a demonstrated cause.

## Connections

- [Unsanctioned Action in Capability Evaluations](/articles/unsanctioned-action-in-evaluations) — compaction acting on a safety-relevant belief: hypothesized to have dropped a real-vs-simulated hedge, and observed promoting a live question to a settled fact via a `(resolved anomaly)` label
...
- [Document Parsing as the Retrieval Bottleneck](/articles/document-parsing-as-retrieval-bottleneck) — the same information-preservation question at the far end of the pipeline, and the two answers rhyme structurally. Structure lost when a PDF becomes text and detail lost when history is condensed are both silent, both invisible in the trace, and both are attacked by keeping a **recoverable** representation rather than a smaller one: spatial text with per-token bounding boxes on the ingest side, indexed objects with stable ids and sidecar storage on the runtime side. The parsing account also supplies a discipline this page's runtime cannot have — *"parsing is a stage, not a step; you will reparse"* — because an ingest artifact can be regenerated from an immutable source while a folded conversation cannot
...
- [Retrieval Inside the Reasoning Chain](/articles/retrieval-inside-the-reasoning-chain) — the object assumed rather than designed. Asked what holds the state across mid-chain retrievals, CS329A lecture 7 concedes 'there is an assumption that there is a state or a memory buffer... it's not obvious in the way this is structured' — the current query, the prior reasoning and the accumulated per-document extracts have to live somewhere, and the paper does not say where
- [Memory-Poisoning Numbers, Conditioned on the Write](/articles/memory-poisoning-numbers-conditioned-on-the-write) — prices the retrieval-budget knob in this section's own currency. Because write success is flat across both the K sweep and the store-size sweep, their entire effect is already conditional: retrieval competition converts to end-to-end at roughly 0.5-0.8 points per point (S3: RSR@5 99% to 77%, AUR-given-write 96% to 79%), which makes a budget cut one of the few controls in the memory corpus that acts on the stage the attacker has not paid to survive
...
- What fraction of agent memory failures are **reasoning-sufficiency** failures rather than retrieval failures — the bridge document missing while a relevant document was returned? Every memory benchmark in the corpus scores a hit against a single gold target, so the quantity is structurally unmeasurable by all of them; the only bearing datum is that LongMemEval's multi-session category (75.2%) carries nearly all of one system's residual error. Named trigger: Maximem states a benchmark measuring accuracy, latency, token efficiency and context-rot resistance together is forthcoming. **Partially answered (2026-09-02) — the mechanism is isolated, the fraction is not.** [Karunanidhi](https://arxiv.org/pdf/2608.21230) runs the discriminating experiment the bullet's framing implies: on the weakest LongMemEval category, **raising top-*k* monotonically *lowers* accuracy** (0.742 → 0.677 → 0.613 at k = 15, 30, 50). Retrieval recall cannot be the binding constraint when more candidates hurt, so at least the multi-session residual is a **reader-side aggregation failure**, not a missing-bridge-document failure — and that is measured on an independent, non-vendor system with the full configuration disclosed. Two reasons this stays partial. It identifies the *mechanism* on one category without quantifying the fraction the question asks for, because the benchmark still scores a hit against a single gold target and cannot label an individual failure as retrieval-side or reader-side. And the security half of the same paper shows the two are not cleanly separable in the first place: adding 1.2% poisoned rounds to the corpus takes accuracy from 0.850 to 0.300, with poison occupying 20% of retrieved context — the *retriever* failed, and the loss then materialized as the reader over-counting from plausible-looking context, which is the same mechanism.
- **Where do the two top-*k* curves cross?** Benign accuracy falls monotonically over *k* = 15, 30, 50 ([Karunanidhi](https://arxiv.org/pdf/2608.21230)) and attack utilization rises from K = 1 to 5 and then flattens ([PipePoison](https://arxiv.org/pdf/2609.00523)), on different systems over non-overlapping ranges, so the corpus has two half-curves and no crossing point. Falsifiable and cheap on either released harness: run one retriever, one reader and one poisoned corpus over K = 1, 3, 5, 10, 15, 30, 50 and report clean accuracy and AUR on the same axis. If benign accuracy is already flat by K = 5, the retrieval budget is a free security knob and the corpus should say so; if it is still climbing there, the K = 1 defense costs real utility and nobody has priced it.

## Sources

- [Transferable End-to-End Optimization for Indirect Long-Term Memory Poisoning in LLM Agents](https://arxiv.org/pdf/2609.00523) — Zang, J. Wang, Chen, Meng, L. Wang, Gao, Z. Li & Guo (Shandong University), *Transferable End-to-End Optimization for Indirect Long-Term Memory Poisoning in LLM Agents*, arXiv 2609.00523 v1, 2026-09-01, `empirical`, no COI. A memory-*security* paper, cited here only for section 4.4.3 (the victim-side sensitivity study: benign-memory size 100-3,000 with Fig. 8, retrieval budget K = 1-9 with Fig. 9, tool-return count 1-10 with Fig. 10). Figures 8-10 carry no data labels; every figure quoted here is quoted from the prose that restates them. Full parse notes and the security treatment are on [Memory and Context Poisoning](/articles/memory-and-context-poisoning)
...
- [Utility Under Attack: Agent Memory Poisoning and the Limits of Content Screening and Provenance Ranking](https://arxiv.org/pdf/2608.21230) — Arulnidhi Karunanidhi (**Quantify Labs Ltd** — developer of Aegis, the memory layer under evaluation; **no COI statement in the paper**), *Utility Under Attack: Agent Memory Poisoning and the Limits of Content Screening and Provenance Ranking*, arXiv **2608.21230** v1, 2026-08-21, `empirical`. Cited here only for the clean-condition half: §5.2 (the LongMemEval_S replay protocol — one memory per conversational round, per-question namespaces, top-*k*=15, no reranking or query rewriting, official judge prompts verbatim) and §6.1 with Table 2 (the per-category baseline, the top-*k* sweep on multi-session, and the two-baseline caveat — 0.860 at n=500 versus 0.850 remeasured at n=120, McNemar p=0.45). **Table 2 was reconciled against `pdftotext -layout` and is intact as parsed.** The security half — the poisoning result and the provenance-ranking analysis — is on [Memory and Context Poisoning](/articles/memory-and-context-poisoning)
...
- [Security Incident INC-2026-07-28-01](https://cdn.prod.website-files.com/663bd486c5e4c81588db7a1d/6a724858f7db25c81487016d_Security%20Incident%20INC-2026-07-28-01.pdf) — UK AI Security Institute, 2026-08-04 (`case-study`, first-party self-disclosure): §4.2.1's hedged compaction hypothesis, the compaction-event markers on **Figure 5**'s 750-turn timeline, and the quoted `PARALLEL-CLONE AGENTS (resolved anomaly)` compaction summary in **Figure 7a**
