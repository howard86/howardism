...
- [Impose Values, Not Disciplines](/articles/impose-values-not-disciplines) — a case where the instruction fails not by compounding but by failing to stick at all: agents told to do strict TDD revert to write-function-then-test regardless, and this page's compliance floor is one of three candidate explanations
- [Latent vs. Deterministic Space](/articles/latent-vs-deterministic-space) — the remedy [Robert C. Martin](/articles/robert-c-martin) draws from the same observation: move the durable rules out of the prompt entirely rather than rewording them
