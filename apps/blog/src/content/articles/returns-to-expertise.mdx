...
- [GDPval Benchmark](/articles/gdpval-benchmark) — the benchmark that deliberately erases the premium: tasks are authored by 10-year professionals with all their tacit context written into the prompt, so the model is graded against an expert on the one axis where the expert's advantage has been handed over. Underspecify the prompt again and the model stops knowing what to work on
