...
- [Intra-Trace Parallel Planning (SPRINT)](/articles/intra-trace-parallel-planning) — the same object, a task DAG, reached from the opposite direction: OrchBench scores a plan graph *before* the workers run, while SPRINT recovers the graph *after* a reasoning trace has already run (by having a second model annotate it) and trains it back into the generator
