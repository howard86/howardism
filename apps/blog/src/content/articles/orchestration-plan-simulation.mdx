...
description: "OrchBench (Ren et al., Fudan / Zhongguancun / QMUL, arXiv
  2607.25656): evaluate a multi-agent orchestration plan without running any
  workers — the planner emits only an assignment + cross-agent-transfer plan
  over a fixed task DAG, and a deterministic simulator scores quality, makespan
  and tokens, correlating r=0.816 with real Claude Code quality at 1.3% of the
  tokens and 10.3% of the wall clock. Findings: transfer coverage dominates
  agent count (Agents-Q decays 0.468 → -0.021 as n goes 10 → 100 while
  Coverage-Q holds 0.95 → 0.61), the agent budget saturates (Amax 16 → 64
  doubles agents, moves score ~0.01), and multi-agent beats single-agent only
  under context pressure (+0.302 quality at 16k, +0.007 at 128k, single-agent
  ahead on 82% of model-problem pairs at 128k). Caveats the paper under-plays:
  the headline r falls to 0.421 (p=0.500) once the weakest planner is dropped,
  and the ablation shows essentially all model separation comes from two modeled
  penalties (missing transfers, lossy compression)"
tag: Concept
