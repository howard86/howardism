...
---
export { default as heroImage } from "../assets/hermes-agent.webp";
...
The separation of `AGENTS.md` (project) and `SOUL.md` (personality) is sharper than Anthropic's `CLAUDE.md` convention and worth comparing — the same role split is implicit in `CLAUDE.md` files in practice but not separately filed. (Will be expanded in a forthcoming [Agent Context Files](/articles/agent-context-files) concept page.)
...
### Development Pace, Measured from Outside (July 2026)

[OpenHands](/articles/openhands)' twelve-month GitHub analysis ([Coding Agents and Technical Debt](https://www.openhands.dev/blog/coding-agents-and-technical-debt), `case-study`, competitor-authored) includes `NousResearch/hermes-agent` and makes it the outlier in both directions. In the year to 2026-07-08: **7,736 merged PRs — the most of the four harnesses compared — ~3.0M lines changed, and ~1.75M lines of current code, the largest codebase in the comparison.** The growth curve is the steepest too: from "a nearly empty repo to roughly 2,000 merged PRs a month within months of getting serious."

Its **68% bug-fix share** (5,288 PRs) is nominally the highest by a wide margin — the other three sit at 16–40% — but the author flags it as probably an artifact of Hermes' labeling conventions rather than a quality signal, and says the classification is heuristic across all four projects. Do not cite the 68% without that caveat. Context and the fork/build-vs-buy argument these numbers serve: [Harness Build-vs-Buy](/articles/harness-build-vs-buy).
...
### Skill-install scanning (August 2026)

Nous Research piloted **SkillSpector** (`NVIDIA/SkillSpector`) as an *optional advisory scan* in the Hermes Agent skill-install flow: PII, Unicode smuggling, script linting, license and security checks, with file-line findings surfaced **before** installation. 29 passing tests; ~1.4–1.5 seconds per skill scan. Advisory, not blocking — but it is the first case in this wiki of a skill registry running static analysis inside the install path, and it pairs with the [Skill Lift](/articles/skill-lift) pipeline's Tier 1 as the security half of a skill-verification gate.
...
- [Harness Build-vs-Buy](/articles/harness-build-vs-buy) — Hermes as the high end of the measured maintenance load: 7,736 merged PRs and ~1.75M lines in twelve months, and the 68% bug-fix-label outlier the source itself discounts
- [OpenHands](/articles/openhands) — the open-source peer harness that published the comparison
...
- [Coding Agents and Technical Debt](https://www.openhands.dev/blog/coding-agents-and-technical-debt) — Rajiv Shah, OpenHands, 2026-07-28 (`case-study`, competitor-authored): third-party GitHub activity data for `NousResearch/hermes-agent`
