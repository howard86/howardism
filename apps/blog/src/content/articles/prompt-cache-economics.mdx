...
- **The compression literature still prices itself in tokens, and now that is documented rather than inferred.** [Not Worth Another Token: Marginal Value Estimation for Efficient Deep Research Agents](https://arxiv.org/pdf/2608.08389) (arXiv 2608.08389, 2026-08-09, `empirical`) is a 40-configuration study of context pruning whose stated subject is *cost* — and "cost" is defined in its Appendix A.4 as summed input plus output tokens across logged phases, with the results table's `Cost` column group reading `# Tokens` + `Runtime (s)`. There is no dollar figure in it. This page's crossover rule is what makes that gap consequential rather than pedantic: whether a token reduction is a bill reduction depends on **which** tokens go and whether the cached prefix is touched, and that paper's own accounting supplies the term — its early-pruning arms remove *output* tokens (billed 5× input on Sonnet 4.6's card, never cache-discounted) faster than input, while its late-pruning arms remove input only and are exactly the prefix-mutating case measured at +40.1% here. Priced on published rates with no caching modelled, its stage ordering *widens* in dollars (early arms 71.1% / 74.5% against 69.5% / 73.3% of tokens) while its late arm *narrows* (7.2% against 11.5%) — so a token-percentage headline is not merely imprecise about the bill, it is wrong by a factor that flips direction with the stage. Full arithmetic on [Tool-Output Pruning](/articles/tool-output-pruning).
- **There is a third lever on the same tax, and it can fight the other two.** This page prices caching against compression; [Shopify's Sidekick account](https://shopify.engineering/sidekicks-continual-learning-loop) (Shopify Engineering, 2026-08-05, `case-study`, first-party and unreplicated) ships **gist compression** — run the same model twice, a teacher with the full system prompt and a student with a short sequence of *learned gist token embeddings*, train the embeddings to match the teacher's output distribution with model weights frozen, and serve the student. Reported: **~6,000 tokens down to ~1,500**, with *"no measured quality loss on the judge."* Its diagnosis of the problem is verbatim this page's: *"an agent's system prompt is long and static. Attention scales with sequence length… A long prompt is a fixed tax on latency and serving cost, paid on every request."* Structurally it is CAPC's prescription taken to the limit — compress **once, query-agnostically, off the request path** — and then past it, because the compressed block is not text, so byte-stability is guaranteed by construction and no compressor ever touches the cached region. Two things keep it from being a straight win on this page's terms. **(1) Its numbers are not comparable with anything here**: TTFT −19%, end-to-end latency −38%, +16% requests/second, +12% output tokens/second and *"roughly 14% fewer GPUs"* are self-hosted serving metrics from a 350 req/min load test, not a billed API cost, and the piece's dollar figures are a separate projection. **(2) On a hosted cache the two levers are antagonists below the threshold**: `r_max_safe = floor(|D|/T)` puts the safe compression of a 6,000-token prefix at **1** on Sonnet 4.6's T ≈ 3,500, and 1,500 tokens is squarely in the hot tier where a byte-identical prefix still misses roughly one call in six. Shopify serves its own weights, so it pays no write premium and the trade does not arise for them — which is exactly the point: **whether learned compression substitutes for caching or collides with it is decided by who owns the serving stack**, and nobody has measured the composition.
...
- [Agent Quality Flywheel](/articles/agent-quality-flywheel) — the third lever on the same static-prefix tax, treated above: gist compression trains the system prompt down to ~1,500 learned embeddings instead of making it cheap to re-read, which is maximally cache-friendly on a self-hosted stack and lands under this page's tier threshold on a hosted one. It also supplies the only production account of a prefix-compression technique whose reported gains are GPUs and latency rather than a bill
...
- Do caching and *learned* prompt compression compose, substitute, or collide? A gist-compressed prefix is byte-stable by construction (ideal for a cache) and small enough to fall through this page's tier threshold (where a byte-identical prefix still misses). The falsifiable version: one static prefix served three ways on one stack — full + `cache_control`, gist-compressed, and gist-compressed + `cache_control` — reporting billed cost or GPU-hours *and* quality, so the composition is measured rather than argued. The only source with gist tokens in production self-hosts its weights and therefore never faces the write premium at all.

## Sources

- [Sidekick's continual learning loop](https://shopify.engineering/sidekicks-continual-learning-loop) — Andrew McNamara & Cody Mazza-Anthony, Shopify Engineering, 2026-08-05, `case-study` (first-party account of the authors' own production system; no replication, no controlled arm). Cited here only for its *"Compress the prompt to serve it faster"* section: the gist teacher/student construction with frozen weights, the ~6,000 → ~1,500 token figure (confirmed by the article's own *System prompt vs gist tokens* diagram, which adds no new number), and the 350 req/min load-test deltas (TTFT −19%, end-to-end −38%, +16% req/s, +12% output tok/s, ≈14% fewer GPUs). **These are self-hosted serving metrics, not billed API cost, and are not comparable with any dollar figure on this page**; the article's $27M → $1M is a modal-hedged projection from "average token costs," not an invoice. Full source treatment on [Agent Quality Flywheel](/articles/agent-quality-flywheel)
- [Not Worth Another Token: Marginal Value Estimation for Efficient Deep Research Agents](https://arxiv.org/pdf/2608.08389) — Kolukuluru, Ashok, Arora, Ciccarelli, Ashok Kumar, Nie, Dernoncourt, Basu, Rossi & Lipka (UMass Amherst / UT Austin / Adobe Research), *Not Worth Another Token: Marginal Value Estimation for Efficient Deep Research Agents*, arXiv 2608.08389, 2026-08-09, `empirical`. Cited here only for its cost convention and its input/output split: **App. A.4** (token-count definition of cost), **Table 5** (the `Cost` column group as `# Tokens` + `Runtime (s)`), and **Table 10** (per-configuration input/output tokens, which is what makes the token→dollar conversion computable). Caching is never mentioned in the paper. The dollar reconstruction above is wiki arithmetic on Sonnet 4.6's published rates with no cache state modelled — illustrative, not a measurement. Per-table verdicts and the three in-paper numeric inconsistencies are recorded on [Tool-Output Pruning](/articles/tool-output-pruning)
