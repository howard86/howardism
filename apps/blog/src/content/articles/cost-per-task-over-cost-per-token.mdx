...
- [GDPval Benchmark](/articles/gdpval-benchmark) — what the GDPval-AA row in the model-selection table above is built on, and where the per-model split it implies gets its evidence: on the original benchmark Claude wins on aesthetics, document formatting and reading PDFs/spreadsheets/presentations while GPT-5 wins on instruction following, correct calculation and text-only work. "Pick the model per task type" is the benchmark's own finding before it is vendor advice
