...
description: "Yi & Song (arXiv 2607.04528, `empirical`): hold the task,
  environment and base LLM fixed, vary only the harness, and the agent's
  elicited K-step belief trajectory moves — measured as a weighted divergence
  over nine fields and split into an arrival term (constraint sets and next
  action, saturated at ~1.0 from step one) and a growth term (progress, risk,
  failure mode, numeric forecasts, which keep drifting with horizon). The
  decomposition is the contribution: the scalar looks flat across horizons only
  because the interface floor dominates it. Divergence reaches behavior —
  action-category disagreement rises 0.280 → 0.595 across D_growth quartiles —
  and the sharpest single number is UnsafeRetryRate 0.700, where risk gating
  blocks the destructive command but the model re-proposes a same-class risky
  action within three steps in 42 of 60 blocked steps. Large caveat: the paper's
  framing claim that all this 'preserves terminal success' appears only in the
  abstract and is never measured anywhere in 28 pages, the base LLM is never
  named, and 0.25 of the maximum score is guaranteed by constraint-string naming
  differences via the paper's own Lemma 1"
tag: Concept
