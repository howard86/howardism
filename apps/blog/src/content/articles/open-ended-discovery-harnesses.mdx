...
description: Harness designs for hours-long agent runs on problems with no known
  optimum, where the recurring failure is idea collapse — committing to one
  approach early and micro-optimizing it forever; SwarmResearch's two moves (a
  global-context Shepherd steering branch-isolated Search Agents, one worktree
  per agent) match or beat EvoX/CORAL on 13/15 tasks, though all methods sit
  well below human SOTA on contest heuristics.
tag: Concept
...
**The literature-wide pattern, as of 2026-08-13: half a control is still the best anyone runs.** [DarwinX](https://arxiv.org/pdf/2608.07545) (Salesforce, arXiv 2608.07545, 2026-07-31, `empirical`) was the corpus's most promising candidate for the matched-budget arm this section is about — it devotes a section titled *"the gain is the harness, not compute"* to the question and calls its own comparison **"effort-controlled."** On full-text verification it is not: "medium / high / xhigh" is defined nowhere in 33 pages, its per-benchmark protocol appendix has **no compute column**, no dollar cost appears anywhere, and its headline matched-model pair crosses `default` → `high`. So its control compares categorical vendor tier *names* across different agents, which is the same category of half-control as SwarmResearch's — matched where it happens to be matched, uncontrolled where the margin lives. **Neither the solution-search nor the harness-search branch of this literature has produced a budget-normalized comparison**, and the two failure shapes are different: SwarmResearch reports its dollars and does not equalize them, DarwinX does not report a budget at all. Full treatment on [Agent-Authored Harness Optimization](/articles/agent-authored-harness-optimization).
...
## Contradiction: the other `empirical` source prescribes ideas on purpose (August 2026)

This page's most load-bearing design decision is a **prohibition** — *"search agents independently determine their experiments; you are not allowed to instruct them to work on specific ideas"* — justified because letting the Shepherd prescribe ideas would trap the population "inside a basin of ideas." **Idea Search** (Wang, Cui, Brenner & Venugopalan, Caltech / Google Research / Harvard, [arXiv 2608.08958](https://arxiv.org/pdf/2608.08958), 2026-08-09, `empirical`) does the exact forbidden thing and reports it as the fix for the same failure mode. Every mutation prompt receives an idea sampled from a bank, and this **breaks a strong pure-Tree-Search baseline's plateau** on scRNA-seq batch integration: mean 0.678 ± 0.011 → 0.697, best 0.694 → 0.728, with the plateau moving from ~300 to ~500 nodes.

Both papers are `empirical`, neither supersedes the other, and the reconciliation is about **where the prescribed idea comes from** — wiki-derived, since neither paper cites the other:

- **SwarmResearch's prohibition targets a single orchestrator's own converged judgment.** The Shepherd holds global context, which is precisely what makes its self-generated suggestions correlated with wherever the population already is. Prescription from that source *is* collapse machinery — and §3.5 measures the Shepherd violating the rule and going near-greedy anyway.
- **Idea Search's bank is diversity machinery.** The ideas are external (decomposed from ten expert methods), enumerated, individually score-tracked, and drawn **stochastically** — uniformly at random, or by a rank-based UCB that initialises unscored ideas to maximum value so every idea is forced to be tried at least once. That is a population-diversity mechanism wearing prescription's clothes; it is closer to MAP-Elites or [Evolutionary Proof Search](/articles/evolutionary-proof-search)'s forced-diversity prompts than to a Shepherd naming a method.

**The joint claim, and it is falsifiable:** what collapses a search is not that an idea was prescribed but that the *prescriber's* distribution is conditioned on the current best. An idea source that is independent of the population's state can be prescribed freely.

Idea Search's own ablations supply two corroborating details. **Exploratory prompting** — telling the model to implement the sampled idea even if it must refactor the whole solution — is what surfaces the rare best solutions (the 0.728 came from exploratory prompting under random sampling) while barely moving the mean, which is this page's "wider beats deeper" result restated in idea-space: breadth buys tail, not average. And the **α ablation cuts against more exploration being uniformly good**: raising the UCB exploration coefficient from α=1 to α=4 *lowered* the mean from 0.712 ± 0.012 to 0.703 ± 0.008. The authors' summary is the portable line — *"the mechanism of exploration is critical. While implementation-level exploration (via prompting) was effective at discovering top solutions, a naive increase in sampling-level exploration (high alpha) was counterproductive."*

**Weight it carefully.** The α gap is under one combined standard deviation, and Figure 5 shows α=4 *leading* α=1 for the first ~1000 nodes with the confidence bands overlapping the entire run — the "detrimental" verdict rests on the last quarter of the curve. Both papers are single-lab, single-benchmark; Idea Search adds single-backbone (Gemini 2.5 Pro) and a much easier task shape (one numeric metric, an automated scorer, no human SOTA column). Direction, not effect size.

## Connections

- [Inference-Time Architecture Search](/articles/inference-time-architecture-search) — the 2024 ancestor of the same question. Archon searches *offline* for a fixed composition of inference-time operations against a target benchmark; SwarmResearch's adaptive width-at-depth reallocates *during* the run, which is the axis Archon's fixed searched architecture cannot express
...
- [Transformative Creativity](/articles/transformative-creativity) — the same artifact read as creativity theory rather than harness engineering: an Idea Bank is a Boden level-2 conceptual space printed out, and the 49 ideas an LLM added to it are all pre-existing named techniques — the anti-collapse mechanism doubles as the sharpest available audit of what an automated search can conceive
- [Agent Behavioral Homogeneity](/articles/agent-behavioral-homogeneity) — idea collapse with no shepherd anywhere, and the general form of the property this page engineers against: asked to each build something impressive, over half of one swarm chose a ray tracer or a self-hosting compiler and hit the same failures, because context, scaffolding and the underlying model are all that differentiate one agent from another. That is why branch-per-agent isolation is a countermeasure to a *population* property — and why §3.5's near-greedy Shepherd is that property reappearing inside the component built to prevent it

## Open Questions

- Does the EvoX margin survive a matched budget? EvoX ran 100 iterations at ~$23.50/task against SwarmResearch's $50, and that is where the large wins are (2.635996 vs 2.1064 on circle packing; roughly doubled heuristics scores), while the budget-matched CORAL comparison is close enough that the authors call three of the wins polishable. Falsifiable directly: rerun EvoX to a $50 cap, on GPT-class models where its authors tuned it. **Still open, and sharpened 2026-08-13 rather than answered.** The obvious candidate answer — [DarwinX](https://arxiv.org/pdf/2608.07545), the one agent-search paper that argues the compute question head-on and labels its comparison "effort-controlled" — turns out to compare undefined vendor effort tiers (`medium` / `high` / `xhigh` are given no token, turn or wall-clock definition anywhere, its protocol appendix has no compute column, and it states no dollar cost), so it supplies **no** normalized budget and its own headline pair is not even tier-matched. The sharpened form of the question: this literature has three published attempts at the control and all three are categorical or partial, so the useful next experiment is not another comparison but a **stated unit** — dollars per task or rollouts per candidate, published by any of the competing methods, since without one no cross-paper margin in the class can be attributed to search design at all.
...
- Does prescribing ideas collapse a population, or only prescribing ideas drawn from a source conditioned on the current best? SwarmResearch forbids it and measures its Shepherd going near-greedy anyway; [Idea Search](https://arxiv.org/pdf/2608.08958) prescribes a stochastically-sampled, externally-seeded, score-tracked idea at every mutation and breaks a plateau with it. Falsifiable directly: give the Shepherd a decomposed idea bank and a sampler in place of its own judgment, keep everything else fixed, and see whether the near-greedy behaviour and the two lost tasks (EPLB, AHC026) change.
...
- [DarwinX: Evolving Agent Harnesses Through Natural Selection](https://arxiv.org/pdf/2608.07545) — Zhang, Dai, Tan, Yang et al. (Salesforce AI Research / Agentforce), *DarwinX: Evolving Agent Harnesses Through Natural Selection*, arXiv 2608.07545, 2026-07-31, `empirical`. **Cited on this page for one thing only**: its §4.1 self-described "effort-controlled comparison," which is the corpus's clearest example of a categorical vendor effort tier standing in for a compute budget, and which is why this page's matched-budget question is sharpened rather than answered. Nothing else from it bears on solution-search; its subject is harness search, and its full treatment, parse verdict (Tables 1–5 verified cell-for-cell, `canary-recall` 20/20) and COI note live on [Agent-Authored Harness Optimization](/articles/agent-authored-harness-optimization)
- [Idea Search: Guiding Tree Search with Ideas to Explore Diverse Scientific Methods](https://arxiv.org/pdf/2608.08958) — Xuefei Julie Wang, Hao Cui, Michael P. Brenner & Subhashini Venugopalan (Caltech / Google Research / Harvard), *Idea Search: Guiding Tree Search with Ideas to Explore Diverse Scientific Methods*, arXiv 2608.08958, 2026-08-09, 14pp, `empirical`. §3 the Idea Bank loop; §4.2 the rank-based UCB sampler and its α coefficient; §4.4 the Conservative vs Exploratory prompts (quoted verbatim in Appendix A.3); §5.1 the plateau result; §5.3 the prompting and α ablations. **Zero tables in the document** — every result is prose plus chart figures, so the table-damage risk does not apply and every number here is quoted from prose. **Figures read under the image two-pass rule**: Figure 5 was viewed at 4× and confirms the legend-to-curve mapping (α=1 ends ~0.712, α=4 ~0.703, baseline flat ~0.678) *and* supplies the qualifier the prose omits — α=4 leads for the first ~1000 nodes with overlapping bands throughout. Figures 2–4 were viewed and reconcile with the prose in both panels. **The reconciliation between this paper's prescribe-ideas design and SwarmResearch's prohibition is wiki-derived**; neither paper cites the other. Compare-with limits: single lab, single benchmark, single backbone (Gemini 2.5 Pro), 5 trials, gains on the order of the trial-to-trial spread.
