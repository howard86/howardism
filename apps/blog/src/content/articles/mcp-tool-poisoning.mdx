...
description: "The MCP Tool Poisoning Attack (TPA) class — an adversarial
  ...
  **Agentjacking** case study (Tenet Security, `case-study`), a *distinct
  ...
  poisoned-tool-metadata), triggering plaintext RCE — and read against MCP spec
  revision 2026-07-28, which leaves the rug-pull intact (per-request negotiation
  re-checks the protocol *version*, not tool behavior; `server/discover` is a
  version probe with no integrity binding; the deprecated-features registry
  lists spec features, not servers) except for one unintended affordance:
  required `ttlMs`/`cacheScope` plus deterministic `tools/list` ordering give a
  client a vetted prior copy and a clean diff, making re-validate-before-use
  cheap without making it required — which catches a plaintext rug-pull and
  still reports ShareLock as Safe"
tag: Concept
