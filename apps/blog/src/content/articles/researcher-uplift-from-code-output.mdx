...
---
export { default as heroImage } from "../assets/researcher-uplift-from-code-output.webp";
...
- [Expenditure Horizon](/articles/expenditure-horizon) — METR's other 2026 run at the same target quantity, from the opposite direction and with the opposite sign. This note **back-solves** researcher uplift from a vendor-reported code-output multiplier through a production function; that one **measures agent output directly** on a real optimization problem and prices it in dollars — and where Kwa lands on ">2× serial researcher uplift," the direct measurement finds autonomous agents worth 1–2 human contributions per $10K run. The two are compatible (hybrid uplift versus autonomous substitution) and together they bound the question from both ends
