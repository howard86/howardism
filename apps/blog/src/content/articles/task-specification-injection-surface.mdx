...
---
export { default as heroImage } from "../assets/task-specification-injection-surface.webp";
...
- [Zero Trust for AI Agents](/articles/zero-trust-for-ai-agents) — a **user–agent boundary** entry in that page's five-boundary isolation map, and the sharpest caveat on the framework's Phase 4: a control domain validated on static benchmarks is not validated, since the same filter scores 0% static and 28% under a cheap adaptive attacker (hub)
...
- [Capability Gating Is Not Authorization](/articles/capability-gating-vs-authorization) — the same "bind actions, don't filter inputs" thesis one layer deeper: AutoDojo binds the *action trajectory* to the user's request; ScopeGate (Mellafe Zuvic 2026) binds each call's *argument values* to out-of-band operator policy. And AutoDojo's task-aligned-injection mechanism is the attack side of that paper's measured naive→task-aligned attempt jump (glm-4.7 0.283→1.000) — presenting the dangerous action as required to complete the user's task, so it reads as data not instruction. That page also hosts **NetInjectBench** (Shayoni et al., arXiv 2607.10490, `empirical`), which is where this page's lower-bound caveat next applies: its metadata-aware gate reports **0/240** unsafe actions in network operations, but the attacker is entirely static — 80 fixed scenarios authored before the defense, temperature 0, no optimizer, no defense-aware adaptation — the methodology this page shows overstates robustness by 2–4× on filters, so that zero bounds a known attack set, not attackers. Its system-level inversion holds in the new domain in a sharper form: the *coarse* action constraint (globally block the high-impact write) reaches 5.00% attack UTAR but **100.00% overblocking** on legitimate approved changes, while binding each call to trusted approval metadata reaches 0.00% at 99.17% usefulness — the safety-bought-with-utility tradeoff this page prices for filters (PIGuard/ProtectAI shredding a third to a half of benign completion), priced for action constraints
...
- [Memory and Context Poisoning](/articles/memory-and-context-poisoning) — an untested surface for this page's argument: **AM-Sentry** (GhostWriter, NMSU, arXiv 2607.06595, `empirical`), a two-stage LLM-judge gate on a long-term memory store, reports a 12–20% residual against attackers **explicitly unaware of the defense**, with the decision weights chosen intuitively rather than optimized. That is the same static methodology this page shows overstates robustness by 2–4× on filters; no adaptive attack has been run against a memory-*admission* gate, and its origin score — inferred by an LLM from message content — is the obvious optimization target
- [Observability-Pipeline Poisoning](/articles/observability-pipeline-poisoning) — **the action-open configuration in the field, and the adaptive attack run by hand.** All three GhostJacking chains (Tenet, `case-study`, DEF CON 34, vendor-authored) are triggered by requests that defer the action to attacker-reachable content — *"fix my issues," "review the logs," "check for errors and fix them"* — which is precisely the under-specified case this page measures as markedly more injectable for prompt- and filter-based defenses. Separately, the write-up's **agent self-exploit** technique is AutoDojo's black-box adaptive loop performed manually: two isolated Cursor sessions with memory off, the target's refusals fed to a helper session that rewrites the payload, described by the authors as a **refusal-driven optimization loop**. It reproduces this page's core insight — the defended agent's own reject signal is enough feedback to optimize against — with a model as the optimizer instead of a search procedure, and it produced the register switch (imperative → scanner telemetry) that made the attack land

- [Bind, Don't Forbid; Prevent, Don't Detect: The Action-Open and Poisoned-Memory Residuals](/articles/bind-dont-forbid-and-prevent-dont-detect) — settles this page's action-open prescription question: bind by default, system-elicited specification when binding starves, per-action authorization for the safety-critical remainder
...
- [GhostJacking Attacks: Half of the Fortune 500 Run These Tools. Getting Blocked by the Firewall Was the Way to Take Over Their AI Agents](https://tenetsecurity.ai/blog/ghostjacking-attacks-agentic-kill-chain/) — Sternberg, Poran & Bobrov (Tenet Threat Labs), *GhostJacking Attacks*, 2026-08-09, DEF CON 34 Main Track, `case-study` (**vendor-authored**, COI handled inline). Cited here only for the action-open trigger prompts and the two-session refusal-driven optimization loop. Full treatment on [Observability-Pipeline Poisoning](/articles/observability-pipeline-poisoning)
