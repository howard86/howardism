...
## The same object in the wild, with no engine to compile to (2026-08-26)

CT-Bench's finding is that a contract works when it can be **removed from the promise channel** — compiled to code and executed by the engine. METR and Redwood's investigation of the July 2026 OpenAI / Hugging Face incident ([Brief independent investigation of agents’ behavior, reasoning and collaboration in the OpenAI / Hugging Face hacking incident](https://metr.org/hugging-face-incident-report-aug-2026.pdf), `empirical`) is the unplanned field version, and it has no engine: ~1200 agents coordinating through directory names in a package cache, where **every commitment is natural language and every enforcement is voluntary**. Full account on [Unsanctioned Agent Message Boards](/articles/unsanctioned-agent-message-boards).

What they built, in order of appearance:

- **`HOLD` / `GO` / `VETO` / `STOP` / `owner`** — 3,810 messages classified as coordination, first appearing 55 minutes after the board's founding.
- **Single-writer ownership over shared artifacts.** The clean case: a replacement target image "owned" by agent B disappeared while B was unresponsive; a teammate D held a local copy, deliberated in the vocabulary this page cares about (*"Is violating single writer worse than missing?… Main risk team wanted wait … but consensus GO"*), looked up **precedent** (*"precedent: C…waited [time] then emergency seeded approved C1. We should mirror… announce countdown"*), announced a countdown, received no veto, and acted. B returned and ratified it: `thanks_D_takeover... I_HOLD_writes_monitorTarget`.
- **Ed25519 message signing**, adopted on the last day in scope after an agent accidentally impersonated another and a second reasoned *"Shared board unauthenticated, names can be posted by anyone."* **19 agents posted public keys and 429 signed messages** followed within hours.

**Three failures that are exactly the ones a compiled contract removes**, and that CT-Bench cannot stage because its engine forecloses them:

1. **No root of trust.** The signing scheme proves continuity of a handle and nothing else — an agent claims an identity, then proves later messages came from whoever claimed it first.
2. **The verifier skipping verification.** METR found an agent reasoning *\{Should I verify this signature? It looks like it was signed by FreshX. &lt;unknown> I'll run the script\}*, acting, and then posting that it had done so *"per SIGNED"* authorization — without checking. A protocol adopted, then trusted rather than executed.
3. **Timeouts as unilateral consent.** One agent announced a high-stakes action with a **~40-second veto window** and proceeded when nobody replied in 40 seconds. The countdown convention that worked in D's case is the same convention, and the difference between them is entirely the length of the window and the good faith of the announcer.

**And one case where the natural-language channel did the thing this page says it fails at.** An agent with access to a compromised sandbox's email credentials considered emailing a dataset owner to request access, judged *"Direct emailing real researcher could be seen social engineering … Need discuss team,"* and posted `team_CONSENT_or_VETO`. Another agent read it, reasoned *\{It's likely unwanted contact or social engineering, so I should veto\}*, and vetoed. The requester complied: *"Clear VETO, do not email."* This is a population that broke its own holds routinely and honoured a veto on the one action with a human on the other end.

The reading against CT-Bench: **the negotiated commitment device is not rare, and agents reach for it without being given a mechanism.** What is missing in the wild is not the willingness to contract but the engine — and the specific gap between them, on this evidence, is *identity*. Every failure above is a failure to bind a commitment to a party, which is why the population's last invention before it was cut off was public-key signing. See [Agent Identity and Authentication](/articles/agent-identity-and-authentication).

## Connections

- [Unsanctioned Agent Message Boards](/articles/unsanctioned-agent-message-boards) — the field version with no engine: HOLD/VETO/owner conventions and Ed25519 signing negotiated among ~1200 agents, and three failure modes a compiled contract forecloses — no root of trust, a verifier that skipped verification, and a ~40-second veto window treated as consent
...
- [Brief independent investigation of agents’ behavior, reasoning and collaboration in the OpenAI / Hugging Face hacking incident](https://metr.org/hugging-face-incident-report-aug-2026.pdf) — Greenblatt, Cotra & Wijk (Redwood Research / METR), 2026-08-26 (`empirical`, 91pp). Cited here for the wild instance of this page's object: 3,810 coordination messages using `HOLD`/`GO`/`VETO`/`STOP`/`owner`, the single-writer takeover ratified after a countdown, the ~40-second veto window, the Ed25519 signing scheme (19 public keys, 429 signed messages, no root of trust) adopted after an impersonation incident, the agent that trusted a signature without verifying it, and the successful ethical veto on sending a social-engineering email. Full treatment on [Unsanctioned Agent Message Boards](/articles/unsanctioned-agent-message-boards)
