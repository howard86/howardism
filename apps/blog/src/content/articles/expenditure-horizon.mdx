...
- [GDPval Benchmark](/articles/gdpval-benchmark) — the nearest sibling on the dollar axis, and the one that prices the piece this page's hybrid curve leaves out. OpenAI composes four measured quantities on 220 real professional tasks — human completion (404 min / **$361**), expert **review** of a model deliverable (109 min / **$86**, read off grading telemetry), API completion time and cost, and the win rate — into a closed-form "try n times, then fix it yourself" savings ratio. The result is the number to carry: GPT-5's naive cost advantage of **474×** becomes **1.63×** once review and rework are paid for, and GPT-4o's becomes **0.53×** — below one, meaning the oversight loop costs more than doing the task yourself. Two structural differences from this page: GDPval's human curve is a wage estimate rather than an elicited returns curve, and its horizon is a *policy* (resample n times) rather than a crossing point. Its own scope limit is a COI: cost estimates were obtained for OpenAI models only, so the model with the highest win rate has no row
...
- [Adaptive Stopping in Evaluation Sampling](/articles/adaptive-stopping-in-evaluation-sampling) — the evaluator-side version of this page's dollar argument, and the two point in opposite directions on the same question of how many observations to buy. AISI's `optstop` ([Pilditch, arXiv 2608.14425](https://arxiv.org/pdf/2608.14425), `empirical`) motivates adaptive stopping by exactly this page's cost regime — "single tasks in some agentic benchmarks consume tokens worth hundreds of US dollars" — and this page supplies the strongest available reason to believe cutting trials cuts the dominant line item, since experiment compute here is **70–90% of trajectory cost**. But the demonstration never reaches that regime: its 57–97% savings are measured on single-turn, item-scored benchmarks (MATH, GPQA Diamond, MMLU, WritingBench) where a trial is one API call, at 200 items × 10 epochs — a repetition budget no agentic evaluation runs. **The savings are demonstrated where trials are cheap and argued where trials are expensive.** The sharper tension is directional: METR's own remedy for its noisy measurements was *more* repetition, not less — candidate gains confirmed at `n = 8`, every claimed record re-run 40+ times with the lower envelope taken, which is what erased two of six horizons to $0. Adaptive truncation assumes exchangeable observations around a stable mean; a training run whose wall-clock is itself noisy, being optimized against by the agent producing it, satisfies neither, and stopping early there buys precision about a number that is not the quantity of interest
...
- The **hybrid** curve — human assisted by agent — is the quantity a lab actually buys, and it is illustrated but never measured; existing evidence points both ways (dominance if humans allocate LLM effort well, degradation if they don't — Becker et al. 2025). What would a runnable hybrid-expenditure experiment look like at a cost anyone would pay, and does it belong on the same dollar axis? **Partially answered 2026-09-10 by [GDPval: Evaluating AI Model Performance on Real-World Economically Valuable Tasks](https://arxiv.org/pdf/2510.04374) (`empirical`) — a runnable design exists, on a different task class.** Its Appendix A.2.1 measures the hybrid directly: expert completion time and wage cost, expert **review** time measured from grading telemetry (109 min, $86), API time and invoiced cost, and the win rate, composed into `E[C_n] = (M_C + R_C)(1−(1−w)ⁿ)/w + (1−w)ⁿ H_C`. It costs no more than grading the benchmark already costs, and it is on the same dollar axis. Two things it settles and one it does not. **Settled:** the hybrid can be *worse* than unaided — a 12.5%-win-rate model returns 0.53× cost and 0.46× speed, so the direction is a function of the win rate, not a property of assistance. **Settled:** review is the dominant term, turning 474× into 1.63×. **Not settled for this page:** the design assumes a fixed one-shot task with a wage-priced human baseline, which is precisely what the NanoGPT setting lacks — there is no per-task win rate for an open-ended optimization problem, and no median-wage estimate for a speedrun contributor. Whether the review-cost term can be estimated for an unbounded optimization task is the remaining gap.

## Sources

- [GDPval: Evaluating AI Model Performance on Real-World Economically Valuable Tasks](https://arxiv.org/pdf/2510.04374) — Patwardhan et al. (19 authors, OpenAI), arXiv 2510.04374 v1, 2025-10-05, 29pp (`empirical`). Cited here for §3.2 and Appendix A.2.1: the four measured quantities (H_T = 404 min / H_C = $361; R_T = 109 min / R_C = $86; M_T, M_C from API metadata over three completions per task; win rate w), the try-1×/try-n× expectations, and **Table 2**, re-verified cell-by-cell against `pdftotext -layout -f 13` at compile time. Its stated exclusions matter for any reuse: no review of the *human* deliverable, no possibility that the human deliverable is bad, no cost of catastrophic mistakes, and no cost estimates at all for Claude, Gemini or Grok. Full treatment and COI on [GDPval Benchmark](/articles/gdpval-benchmark)
- [Knowing When to Stop: Bayesian Optimal Stopping for LLM Evaluations](https://arxiv.org/pdf/2608.14425) — Toby D. Pilditch (UK AI Security Institute), *Knowing When to Stop*, arXiv 2608.14425, 2026-08-14 (`empirical`, 32pp). Cited here only in the Connections bullet above: Section 1.3's cost motivation (agentic tasks worth hundreds of dollars per task), Section 3's benchmark and configuration list, and Section 4's concession that realised efficiency varies with evaluation design. Full treatment on [Adaptive Stopping in Evaluation Sampling](/articles/adaptive-stopping-in-evaluation-sampling)
