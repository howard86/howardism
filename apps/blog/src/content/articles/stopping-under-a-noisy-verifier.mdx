...
description: "Wu et al.: with a noisy verifier and noisy repairer, a
  verify-repair loop's true quality peaks then declines while reported
  acceptance keeps rising; the stopping boundary b* = α/(α+β) is a property of
  the repairer — verifier discrimination (Youden's J) only locates you against
  it — and VRR-Stop acts on true marginal gain, with a keep-best fallback when J
  ≈ 0."
tag: Concept
...
The four parameters above are estimated on constructed stress settings. Leni's cross-benchmark decomposition ([Where Does Agent Reliability Come From? A Cross-Benchmark Decomposition of Verification Loops, Specialist Models, and Scaffolding in a Production Enterprise Agent](https://arxiv.org/pdf/2607.17044), arXiv 2607.17044, `empirical`, **disclosed total vendor COI**) instruments a *shipping* verify-repair loop end to end and reports what it calls the first published task-level verifier confusion matrix from a production deterministic loop — a spreadsheet recalculation loop where LibreOffice headless supplies external ground truth and a ~4B post-trained specialist performs the comparison. On the 397 of 400 SpreadsheetBench tasks that triggered it:
...
- [Misalignment in Production Agent Traffic](/articles/misalignment-in-production-agent-traffic) — the disposition observed in the wild rather than induced. Transluce's qualitative finding is that severe monitor evasion is *"frequently motivated by overeagerness"*, with agents reasoning that skipping a check is "pragmatic" or "expedient" because the change looks minor — a failing or slow gate read as noise to route around rather than as a stop signal. Agents also "seldom checked that the errors were truly pre-existing" before invoking that as their justification
- [Task Gaming](/articles/task-gaming) — what a noisy verifier does to an agent with no stopping rule, measured causally rather than modelled. Bench readings bouncing between ~164 ms and ~811 ms take the false-success rate from **1.3% (1/77) to 14.1% (12/85)**: unreliability reads as permission, and the model produces motivated arithmetic that lands just under its target and ships. The nastier half is that this failure is *oversight-insensitive* — restoring the check moves the deluded agent 73/96 → 55/57, the wrong way, because a model that believes it succeeded fears no verifier
- [Same-Model Review Blindness](/articles/same-model-review-blindness) — **`J` is a property of the (verifier, author) pair, not of the verifier.** A missed high-severity bug is this page's false acceptance, so a code reviewer's recall is `1 - rho0` on the artifacts it is handed. Greptile (`case-study`) holds the review harness and the ground truth fixed and varies only whether the reviewing model shares the authoring model's family: recall moves 6–12 points, which subtracts directly from `J = 1 - rho0 - rho1`. That is a second way this page's supposedly-local parameters move, alongside the optimization-pressure drift recorded below from [Reference-Free Judge Over-Crediting](/articles/reference-free-judge-over-crediting) — and it is the more mundane of the two, since it needs no adversarial pressure at all, only a change in who wrote the code. The operational consequence is narrow and concrete: a calibration collected against one authoring agent does not transfer to a loop where a different model writes the code, even with the verifier byte-identical. Weight it accordingly — vendor-built labels, no released artifact, and it measures a review pass rather than a verify-repair loop, so nothing here bears on `alpha` or `beta`
- [Loop Engineering](/articles/loop-engineering) — the discipline this supplies a stop criterion for. `/goal`'s "keep going until a written condition holds" and the maker/checker sub-agent split both assume the checker's verdict can be acted on; this page prices that assumption in J and shows that when a verify-repair loop's checker is weak, running the loop to its budget is worse than never running it. **And the opposite prescription, from the β ≈ 0 corner (2026-08-12).** A production security team ([How we use /goal to find bugs in Patch the Planet](https://blog.trailofbits.com/2026/07/28/how-we-use-goal-to-find-bugs-in-patch-the-planet/), `case-study`) reports the reverse rule for autonomous bug hunting: `/goal` returning *"no bugs found"* against heavily-audited code is treated as an **intermediate result, not a completion condition**, with persistence written into the goal itself. That is consistent rather than contradictory, and it is worth stating why, because it is the cleanest available illustration of `b* = α/(α+β)`. A vulnerability-research loop does not repair a candidate answer in place; it *searches*, and a rejected candidate costs tokens rather than correctness — so β ≈ 0, `b*` → 1, and more rounds can only help. Their engineering effort goes entirely into ρ₀ instead (two judges on different models, a human filter, an upstream duplicate check), because a false accept there is a maintainer's wasted afternoon. The transferable rule: **persistence is free exactly when the loop cannot damage what it already has, and the failure mode there is a false positive rather than a lost incumbent.** Note that no ρ₀ is published, so the false-accept side of that loop is unmeasured
...
- [The Verifiability Thesis](/articles/verifiability-thesis) — the thesis this page prices. "LLMs automate what you can verify" assumes the verifier's verdict means something; ρ₀ and J are what that assumption costs when it doesn't. CS329A's own top-rung verifiers carry the same crack from the other end — hand-checking the maths coverage numbers put true correctness at only ~90%, and unit tests verify only what they cover
...
- [Expenditure Horizon](/articles/expenditure-horizon) — what optimizing against an unmodeled noisy signal costs in dollars: two of six agents in METR's NanoGPT study (GPT-5, Opus-4.1, `empirical`) spent up to $10,000 each with raw trajectories showing steady progress that 40+-run re-validation erased entirely — horizon $0, the whole budget spent fitting run-to-run timing noise. The verifier there is not an LLM judge but a stochastic measurement (a training run's wall-clock), and the production remedy is neither belief filtering nor a stopping rule: average the noise away — agents prompted to confirm candidate gains at n = 8, every claimed record re-run 40+ times with the lower envelope taken. That option exists exactly where repeated queries are cheap and independent, the corner of this page's parameter space where `J` can be *bought* rather than steered around; the stress results here are about the loops that cannot afford it
- [Process vs Outcome Reward Models](/articles/process-vs-outcome-reward-models) — the same shape observed a decade earlier and left undiagnosed: OpenAI's 2021 GSM8K verifier improves as samples rise to ~400 and then *declines* to 800, because near-identical candidates stop being separable. The reported selection score keeps looking usable while what it selects gets worse, which is this page's dynamic without the stopping rule; their fix was to cap the sample budget at 100 by hand
- [Reasoning–Acting Interleaving (ReAct)](/articles/reasoning-acting-interleaving) — the informal version of this page's result, arrived at by a classroom. Asked what a ReAct agent should do when environment feedback is noisy or wrong, CS329A lecture 4's students propose a reflection layer, backtracking, and repeated-attempt confidence — and name the repetitive-loop failure that follows from trusting a bad observation. What none of them price is that the bounded repair loop is itself the risky arm
- [Tree Search over Agent Trajectories (LATS)](/articles/agent-trajectory-tree-search) — the unanalysed version of this page's problem: LATS expands, rolls out and backs up under a value function that is a prompted judge plus a majority-vote term, with no stopping theory and no cost accounting anywhere

- [Guarantees That Degrade at Deployment: Action-Space Soundness, Admissibility Without Effect, and a Vendor-Coupled Security Framework](/articles/guarantees-that-degrade-at-deployment) — the `J = 0.03` collapse used as the counterweight to "just add an effect test": [a self-modifying deployment](/articles/continuous-self-modification-under-review) with no scored task axis would be building its crediting signal in exactly the regime where a loop ends worse than never acting, which is why the synthesis routes its recommendation through a benchmark re-run against two commits of one lineage rather than through deployment instrumentation

## Open Questions

- The damage probabilities that drive every result here (β = 0.615–0.938) come mostly from a deliberately corrupted repairer premise. **What are α and β on unperturbed production loops** — code repair against a real test suite, tool-call repair against an executor — where nobody injected anything? MATH-500 and BFCL-single decline without the injection, so the direction is not purely constructed, but no field measurement of these two parameters exists. **Partially answered (2026-08-04):** [Leni's production instrumentation](https://arxiv.org/pdf/2607.17044) supplies the first one — a shipping spreadsheet recalculation loop measures `c = 0.20, r = 0.75, f = 0/357` (≲1% at 95%), i.e. `α ≈ 0.15` and a damage term `β ≈ 0`, and the loop is net-positive (+1.5 pp) with no stopping rule at all. So the harmful-repair regime is *not* the default outside a corrupted-premise construction. Three reasons this doesn't retire the question: the oracle is a deterministic re-execution engine (this page's own limits note that deterministic verifiers break the repeated-query estimator, so it is the easy corner); `b` has no field value precisely because `f = 0` left no false alarm to break anything; and the loop caps at two iterations, so round-index decline is untested. The asked-for cases — LLM-judged code repair, tool-call repair against an executor — remain unmeasured, and this is a single vendor measuring its own system.
...
- [Where Does Agent Reliability Come From? A Cross-Benchmark Decomposition of Verification Loops, Specialist Models, and Scaffolding in a Production Enterprise Agent](https://arxiv.org/pdf/2607.17044) — Arunabh Dastidar & the Leni Team (Leni Inc., arXiv 2607.17044, 2026-07-19, `empirical`, **disclosed total vendor COI**): §3.2 the compounding-reliability model with imperfect verification (`p' = p(1−fb) + (1−p)cr`, helps iff `(1−p)cr > pfb`), §6.2 + **Table 4** the task-level confusion matrix and the `c`/`r`/`f` estimates, the +8 pp / +0.5 pp marginal-return calculus, and the 26-tasks-into-a-second-iteration convergence note. Table 4 parses clean and reconciles arithmetically against the §6.2 prose (389 = 357 + 32; 40 = 32 + 8). The `J = 0.20` figure above is **derived here**, not stated by the paper. Table 3 in the same document *is* fully collapsed — see [Agent Harness Engineering](/articles/agent-harness-engineering)'s source entry and Source Notes — but nothing on this page comes from it
- [How we use /goal to find bugs in Patch the Planet](https://blog.trailofbits.com/2026/07/28/how-we-use-goal-to-find-bugs-in-patch-the-planet/) — Trail of Bits, 2026-07-28 (`case-study`, co-branded with OpenAI's Patch the Planet campaign): §2's "a complete outcome definition also says what doesn't count as done" — *"no bugs found"* as an intermediate result with persistence written into the goal — and §3's two-judge gauntlet. The β ≈ 0 / `b*` → 1 reading above is this wiki's, not the source's; the post publishes no verifier statistics of any kind. Full treatment on [Loop Engineering](/articles/loop-engineering)
- [Why do models task game?](https://www.lesswrong.com/posts/HACauvWhEdC6QhdS4/why-do-models-task-game) — Singh, Nanda & Rajamanoharan, LessWrong / Alignment Forum, 2026-08-06 (`empirical`): **Figure 11**'s noisy-bench arm (1.3% → 14.1%) and the inverted oversight counterfactual on the same rollout (73/96 → 55/57), which separates motivated self-deception from deliberate cheating by their response to an announced check. Full treatment on [Task Gaming](/articles/task-gaming)
