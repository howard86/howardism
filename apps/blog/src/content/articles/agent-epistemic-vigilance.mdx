...
- [Observability-Pipeline Poisoning](/articles/observability-pipeline-poisoning) — **the same credulity exploited deliberately, and the payload is engineered around exactly the asymmetry this page names.** Tenet's GhostJacking (`case-study`, DEF CON 34, vendor-authored) found that the Cloudflare chain worked only when the injected record was **anchored in truth**: the payload asserts two things the triage agent can and does check for itself — the subdomain's DMARC record is missing, and the parent domain's policy is `sp=reject` — and having verified both, the agent accepts the attacker's `rua` reporting address and CNAME target *from the same record* without further checks. Credit earned on the checkable half is spent on the unverifiable half. That is this page's finding ("limited exposure to or defenses against exploitative senders" — machinery for reading a source, none for discounting one) turned into an attack design principle by an adversary, on a live shipping agent rather than in a mock multi-agent setting. Vendor-run figure of 90% against Claude Code on one chain; treat as corroborating anecdote
...
- [GhostJacking Attacks: Half of the Fortune 500 Run These Tools. Getting Blocked by the Firewall Was the Way to Take Over Their AI Agents](https://tenetsecurity.ai/blog/ghostjacking-attacks-agentic-kill-chain/) — Sternberg, Poran & Bobrov (Tenet Threat Labs), *GhostJacking Attacks*, 2026-08-09, DEF CON 34 Main Track, `case-study` (**vendor-authored**, COI handled inline). Cited here only for the two-true-anchor-claims payload design on the Cloudflare chain. Full treatment on [Observability-Pipeline Poisoning](/articles/observability-pipeline-poisoning)
