...
- [Covert Capabilities](/articles/covert-capabilities) — the capability an evasion-trained organism is built to exercise; Anthropic's fifth stress-test organism used external memory to make its misalignment context-dependent and defeated the audit's first pass
- [Structured Safety Case (Claim Decomposition)](/articles/structured-safety-case) — the sampling problem made load-bearing: an alignment audit validated on five organisms is validated against the failure modes someone already imagined
...
- [Interference Weights](/articles/interference-weights) — the same measurability-for-representativeness bargain, one level down. There the model is tiny (2.9M parameters, one layer, no normalization) rather than quirk-planted, chosen precisely so that an almost-ground-truth measure — the actual change in loss when a weight is ablated — is affordable at all; and the authors concede up front that neither of their metrics scales to frontier models. Both constructs buy a legible answer key and then owe a transfer argument they cannot yet make
