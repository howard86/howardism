...
description: "Rajan: omission — a decision-critical fact silently missing from
  ...
  knobs survive. The nine layers stop short of one boundary: a commitment lost
  at request-to-plan leaves every byte intact, and the multilingual-planning
  source's typed task representation is this page's canary used prophylactically
  with the free measurement never taken."
tag: Concept
...
## The boundary the nine layers do not cover

Every layer in the table is about a **payload**: a record, a tool result, a prompt string, a token ID, a cache-resident span, a generated answer. The taxonomy's question is always *did this fact survive from where it was to where it was needed*. There is a loss with the same signature — silent, fluent, complete-looking output — that no L0–L8 tap can see, because nothing about the payload went missing.

Pahuja, Brokman, Hofman et al. ([An Actionable Diagnosis of Multilingual, Multi-Agent Planning Failures](https://arxiv.org/pdf/2608.03735), arXiv 2608.03735, 2026-08-04, `empirical`) name it at the **request-to-plan boundary**: a **planning-grounding failure** is "a mismatch between the user request and the generated plan such that, even if the plan were executed correctly, the resulting execution would not satisfy the original request." Their five recurring forms are the commitments a plan drops — the target entity, the required source (or its post-retrieval filter), the time period, the operation sequence, the answer format. Here the thing that dies is not a fact in a document but a **constraint in the request**, and it dies inside a transformation that is *supposed* to rewrite its input, which is why the deterministic/behavioral split does not classify it. There is no byte to diff: a plan that substitutes the wrong arXiv paper is a well-formed plan, every tool call succeeds, every payload arrives intact, and the report answers a different question. One worked case is the read-side loss this page would otherwise own, arriving one level up — the query asks for the year written *in an image* on a page, the plan scans the page's text, metadata, headers and references instead, and returns a real year from the wrong modality. Not an L1 pagination stop; an L(−1) instruction stop.

**And the paper builds the tap without recognizing it as one.** TART is a typed representation of the request — `entities`, `time_constraint`, `source_constraint`, `attachment_type`, `operations`, `answer_type` over closed vocabularies — extracted before planning and injected into the planner and every downstream agent so the commitments "remain in view at each stage of the pipeline." That is this page's canary protocol used **prophylactically instead of diagnostically**: rather than planting a marker in order to count what a stage dropped, it re-states the marker as payload so the stage has nothing to drop silently. The measurement is then free and nobody took it. With a typed representation of the request in hand you can **diff the plan against it field by field and count which commitments the plan failed to carry** — exactly the L0–L3 property (an exact-match check at a boundary, no judge, per-request) on a boundary the nine layers class nowhere. The paper instead scales its categorization with Claude Opus 4.8 as an LLM judge, whose per-language calibration it never validates. It built the instrument that would have made its own diagnosis judge-free and then hired a judge.

Two honest limits on that idea, both this page's own. The diff inherits the **paraphrase artifact** — a plan that preserves a constraint in different words scores as a loss under substring matching, which is the same reason OR 3.65 on this page's paraphrase row is partly a measurement artifact and says so. And the field extraction is itself an LLM call, so the "checkpoint" is not deterministic software the way T1–T5 are; it is a cheap, single-purpose, inspectable call, which is a weaker guarantee than a byte comparison and a much stronger one than reading a whole trace.
...
- [An Actionable Diagnosis of Multilingual, Multi-Agent Planning Failures](https://arxiv.org/pdf/2608.03735) — Pahuja, Brokman, Hofman, Nizri, Vishna, Goldfarb-Tarrant, Marchisio, Kojima & Vainshtein (Fujitsu Research of Europe / Cohere / Fujitsu Research), *An Actionable Diagnosis of Multilingual, Multi-Agent Planning Failures*, arXiv 2608.03735, 2026-08-04, `empirical`. Cited here only for §1 (the planning-grounding definition), §3.1 (the five dropped commitments), §3.3 + App. B (the TART schema and its injection into planner and downstream agents), App. A Example 2 (the image-versus-page-text source-modality case) and App. G (Claude Opus 4.8 as the categorization judge). No number on this page comes from one of its tables. Full treatment, parse notes and COI on [Automated Failure Attribution](/articles/automated-failure-attribution)
