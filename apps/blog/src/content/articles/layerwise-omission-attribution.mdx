...
description: "Santhiya Rajan (arXiv 2607.22448, `empirical`): omission — a
  decision-critical fact silently missing from an agent's answer — is a property
  of the whole pipeline rather than of the model, and can be assigned to one of
  nine layers (L0 ingestion through L8 agent loop) by checkpoint taps that
  exact-match a unique canary at every boundary. L0-L3 are deterministic
  software losses counted exactly with no judge; L4-L8 are behavioral, estimated
  by contrast. Across 75,476 synthetic trials the waterfall implies benchmark
  omission O_R = 0.574 with 73.4% of loss in L0-L3 — but Phase A deliberately
  injects those faults, so that split describes the designed allocation and not
  production prevalence, and the paper says so four times. What survives the
  caveat: the taxonomy, the canary-tap method, and three operator-controlled
  knobs that all raise omission (context length OR 7.43 at 32k vs 2k, q4 KV
  cache 2.25, scaled RoPE 1.99)"
tag: Concept
