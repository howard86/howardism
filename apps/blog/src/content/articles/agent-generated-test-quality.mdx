...
## The instrument that would close the measured gap (Martin, 2026-08)

Both studies above measure whether agent tests *reach* the code — coverage of changed lines, presence of a test change, miss rates by construct. Neither measures whether the tests that do reach it actually **constrain** it, and coverage cannot: a test that executes a line without asserting anything about it counts as covered.

[Robert C. Martin](/articles/robert-c-martin) runs the instrument that closes exactly that gap, as a mandatory stage in his pipeline ([Uncle Bob on Software Fundamentals in the Age of AI](https://www.youtube.com/watch?v=zcLPGC-tvgk), 2026-08-19, `practitioner-opinion`). Mutation testing flips operators throughout the source and requires the suite to fail on every flip; a flip the suite survives is a **surviving mutant** — a line that is covered but unconstrained. His hardener stage runs it to exhaustion and is "absolutely merciless."

Three ways this bears on the numbers here:

- **It converts this page's coverage findings into a floor rather than an estimate.** If 61.5% of changed lines are executed in Java and 27.0% in Python, the fraction actually *pinned* is at most that and plausibly well below it. Nobody has run a mutation score on the AIDev corpus, and it is the single cheapest addition to either study.
- **The error-handling result is the one it would move most.** Up to 86.0% of error-handling constructs go unexercised. Error paths are also where assertion-free coverage is most common — a test that triggers an exception without checking what was raised or what state survived. A mutation gate treats both failures identically.
- **It is a gate, not a metric.** Martin's use is not measurement but enforcement: the agent loops until no mutants survive. That is the same shape [Deterministic Engineering for Agent Code Review](/articles/deterministic-agent-code-review) argues for and the same one [Reviving Impractical Quality Tools](/articles/reviving-impractical-quality-tools) explains the availability of — the technique was sound in 2000 and unaffordable because a human had to kill the mutants.

He pairs it with a claim this page has evidence for: agents will not do TDD however they are instructed, reverting to write-function-then-test ([Impose Values, Not Disciplines](/articles/impose-values-not-disciplines)). His response is to stop specifying the ordering and enforce the outcome, which is exactly the substitution the numbers above would recommend if anyone had measured it.

## Connections

- [Reviving Impractical Quality Tools](/articles/reviving-impractical-quality-tools) — mutation testing as the affordable-now instrument that measures constraint rather than coverage
- [Impose Values, Not Disciplines](/articles/impose-values-not-disciplines) — why Martin enforces the outcome instead of the test-first ordering agents will not hold
- [Robert C. Martin (Uncle Bob)](/articles/robert-c-martin) — the practitioner running the gate
...
- [Open Source Under Agent Contributions](/articles/open-source-under-agent-contributions) — [DHH](/articles/dhh)'s claim that instructed agents outclass the median contributor is about process compliance (tests written, why documented), not about the plumbing deficits this page measures
...
- [Uncle Bob on Software Fundamentals in the Age of AI](https://www.youtube.com/watch?v=zcLPGC-tvgk) — Robert C. Martin with [Matt Pocock](/articles/matt-pocock), 2026-08-19 (`practitioner-opinion`; auto-caption transcript): mutation testing run as a mandatory pipeline gate, and the observation that agents revert from instructed TDD to write-function-then-test
