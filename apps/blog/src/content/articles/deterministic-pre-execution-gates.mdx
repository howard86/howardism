...
description: "Reddy, Challaram & Basu (arXiv 2607.07405, `empirical`): silent
  policy violations on policy-permissive tools are a distinct failure class —
  78% of τ²-bench airline failures are wrong final states with no tool error —
  and a four-gate suite of deterministic, read-only predicates over the proposed
  call and current DB state, evaluated before the write executes, raises
  gpt-4o-mini success 29.6% → 42.0% (+12.4pp, replicated +12.3pp on 15 disjoint
  seeds) with the lift concentrated on the 26 tasks where gates fire. The
  boundary is drawn by two negative controls (self-enforcing retail tools, BFCL)
  and five admission axes; one gate carries the lift at 100% precision over 161
  fires while another sits at 5%, so gate precision must itself be audited. The
  claim worth keeping: action-boundary enforcement can *raise* task success
  rather than only bound its safety cost, because the blocked action was the one
  that would have silently corrupted the state"
tag: Concept
