...
description: "Kranti & Vajjala (arXiv 2607.12885): the presence and placement of
  ...
  Zhou (arXiv 2607.05904) then puts the generosity under optimization pressure:
  self-play against a reference-free judge drives its pass rate 0.716→0.938
  while anchor-verified accuracy holds at 0.209→0.202 (gap 0.735), the
  manufactured errors transfer across judge families and scales, a strict
  three-family ensemble still accepts 55% of them, and no monotone aggregation
  escapes — because the decisive variable is not the judge's capability or
  whether it sees the candidate but whether it commits an answer of its own
  first (false-positive rate 0.719 → 0.012, and zero when used as the training
  reward)"
tag: Concept
