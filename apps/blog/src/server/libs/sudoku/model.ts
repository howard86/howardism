...
export default class Sudoku {
  ...
  static readonly ARRAY_FROM_ONE_TO_NINE = new Array(9)
    .fill(0)
    .map((_, i) => i + 1);
  ...
  get isSolved(): boolean {
    ...
    return Sudoku.ARRAY_FROM_ONE_TO_NINE.every(
      (i) =>
        Sudoku.ARRAY_FROM_ONE_TO_NINE.every((num) =>
          this.getRow(i).includes(num)
        ) &&
        Sudoku.ARRAY_FROM_ONE_TO_NINE.every((num) =>
          this.getColumn(i).includes(num)
        ) &&
        Sudoku.ARRAY_FROM_ONE_TO_NINE.every((num) =>
          this.getBlock(i).includes(num)
        )
    );
  }
  ...
  private validateInput() {
    ...
    if (invalidInput.length > 0) {
      throw new Error(
        `Incorrect input with invalid values=[${invalidInput.join(", ")}]`
      );
    }
  }
}
